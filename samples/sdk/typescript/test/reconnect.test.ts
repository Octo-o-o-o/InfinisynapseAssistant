...
test("reconnect.enabled=false 时流断开即判错", async () => {
  ...
  const res = await runTask(fake as never, { text: "hi", taskId: "t", reconnect: { enabled: false } });
  assert.equal(res.status, "error");
  assert.equal(res.reconnects, 0);
});

test("用户级广播里的其他 taskId 事件不会进入当前任务状态", async () => {
  const calls: string[] = [];
  const fake = {
    async openEvents() {
      return streamFrom([
        // 其他任务先发完成信号；若未过滤，当前任务会被提前判完成。
        'event: message.add\ndata: {"taskId":"task-other","message":{"ts":1,"say":"completion_result","text":"错误任务"}}\n\n',
        'event: message.add\ndata: {"taskId":"task-good","message":{"ts":2,"text":"正确任务"}}\n\n',
        'event: message.add\ndata: {"taskId":"task-good","message":{"ts":3,"say":"completion_result"}}\n\n',
      ]);
    },
    async newTask() { return { success: true }; },
  ...
  const res = await runTask(fake as never, {
    ...
    onEvent: (ev) => forwarded.push(ev.event),
  });

  assert.equal(res.status, "completed");
  assert.equal(res.finalText, "正确任务");
  assert.deepEqual(forwarded, ["message.add", "message.add"]);
