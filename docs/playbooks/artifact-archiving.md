...
列表 API 不应默认返回完整 `previewText` 或 artifact 正文。列表响应建议只给 metadata 与 `hasPreview`：文件名、类型、大小、checksum、visibility、`archive_status`、`is_final`、`is_required`。预览正文、消息全文和大 JSON 应放在单独 preview/detail 接口里做权限校验、限流和大小保护。
...
- 生产启用前做对象存储 preflight：用生产同一组环境变量执行小文件 PUT/GET/DELETE，确认 bucket、endpoint、region、path style、权限和删除能力都正确。preflight 失败时不要打开面向用户的长任务下载能力。
...
- 生产下载缺 `storage_key`、归档对象不存在或对象超出下载上限时，返回可解释错误并触发补偿；不要为了临时可用而长期回源 provider workspace。开发和 smoke 可以保留 provider fallback，便于调试。
...
- artifact 列表是否只返回 metadata/hasPreview，而不是默认下发完整预览正文或消息全文？
- storage key 是否避免 PII、未清洗文件名和 path traversal？
- 生产上线前是否用同一环境变量做过对象存储 PUT/GET/DELETE preflight？
