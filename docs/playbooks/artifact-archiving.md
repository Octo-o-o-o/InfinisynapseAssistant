...
6. 对下载内容计算 `sha256`、`size`、`contentType`，上传到自有对象存储或文件服务。
...
- 上传成功但数据库写失败时，保留可对账的临时 key 或确定性 key；数据库成功但上传失败时标记 `missing` / `archive_failed`，不要让下载端返回空内容。
...
- 归档 worker 是否幂等，能处理重复完成事件、恢复和 backfill？
