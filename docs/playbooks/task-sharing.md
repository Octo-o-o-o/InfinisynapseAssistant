...
公开 = **任何拿到 `taskId` 的人都能无鉴权读到该任务的全部消息和文件**。只公开你愿意完全公开的任务；含用户上传材料、内部数据、客户资料或闭源项目材料的业务任务，默认不要直接 `setShare`，应生成脱敏副本或自有导出页。
...
| 从一开始就只使用公开资料的任务 | 含内部数据、上传的敏感资料 |
| 营销/展示用结果页 | 输出里可能带密钥/凭据/内部链接 |
| 给无账号用户看的只读结果 | 只想公开最终摘要，但不想公开完整消息和 workspace 的业务任务 |
...
成熟业务产品更常见的是"原始任务私有，发布脱敏结果"：

1. 原始 InfiniSynapse task 保持 private。
2. 业务后端读取 workspace，只挑选允许公开的产物或主张。
3. 做 DLP/PII/secret/客户名/内部链接检查，生成脱敏副本。
4. 人工确认 redaction diff。
5. 发布到自有对象存储、CMS、报告页或 publish workspace，并用自有权限、过期时间、访问日志和撤销能力管理。

不要为了省事把含上传材料的原始 task 设为 public，再靠前端隐藏文件树或 ZIP 入口。这不是访问控制。
...
- 只想公开最终报告，却把完整原始 task 公开，导致消息、文件树、ZIP 都可读。
- 公开后不给用户撤销入口。
...
- 如果只想公开部分内容，是否走了自有脱敏导出，而不是 `setShare` 原始任务？
