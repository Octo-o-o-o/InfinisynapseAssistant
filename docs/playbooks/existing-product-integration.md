...
轻量直连 LLM 与 InfiniSynapse 长任务的完整分流规则见 [llm-routing.md](llm-routing.md)。成熟产品和全新项目都按工作负载分流：非 agentic 短链路默认直连 LLM，agentic 长任务默认走 InfiniSynapse。
...
上线前要有独立于 pre-commit 的 production preflight。它至少检查生产 feature flag、API Key、队列/worker 可用性、对象存储 PUT/GET/DELETE 探针、InfiniSynapse auth/config 探针，以及需要长任务 worker 的功能是否真的有 worker 在线。preflight 失败时不要打开功能开关，也不要把"本地测试通过"当成生产依赖可用。
...
- `waiting_user` / `WAITING_APPROVAL` 要有明确 TTL。超时后用条件状态转移认领，按需调用 `cancelTask`，再尝试 workspace salvage；如果没有完整产物，释放 active user/concurrency 占位，做幂等退款或用量补偿，并把任务标记为取消或失败。不要让待审批任务永久占用并发名额。
...
- 创建长任务、approve 执行、重新入队、恢复 cron 和 backfill 都要用数据库条件状态转移或等价分布式 claim；不要只靠同进程内存锁。claim 成功后再触发外部副作用，claim 失败说明已有其它 worker 接手。
- 面向用户的高成本入口要在入队前做 per-user/per-org rate limit、active task 并发限制和 feature flag 检查。不要等 `newTask` 已经发给 provider 后才拒绝请求。
...
完整 manifest、归档级别、失败策略和 backfill 标准见 [artifact-archiving.md](artifact-archiving.md)。本节只保留成熟产品接入时的最低边界。
...
任务列表和 artifact 列表 API 默认只返回 metadata：文件名、类型、大小、状态、是否有预览、是否必需、是否最终产物、归档状态等。`previewText`、消息正文和大 JSON 只在单独 preview/detail 接口按权限读取。这样能避免列表页泄露长文本/敏感片段，也避免把长报告塞进每次轮询或列表响应。
...
同样的 claim 语义也适用于邮件、站内信、Slack/webhook、外部 CRM 写入等通知或触达。发送前先写入 `sending` / window claim；provider 明确失败时可以释放或标记可重试，provider 已接受后即使本地 finalize 失败也不要释放 claim，否则恢复任务可能重复触达用户或外部系统。
...
- 是否有 production preflight 覆盖 API Key、队列/worker、对象存储和 InfiniSynapse 探针？
- 高成本入口是否在入队前做了 rate limit、并发限制和 feature flag fail-closed？
- `waiting_user` / `WAITING_APPROVAL` 是否有 TTL、条件认领、取消、salvage 和补偿路径？
