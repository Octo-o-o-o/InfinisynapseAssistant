...
| `final_artifacts` | 最终产物路径；成熟产品建议同时保存 provider path 和自有 storage key |
...
6. 完成后后端读 `getTaskWorkspace` + `downloadTaskFile`，落库产物路径；正式产品的关键交付物复制到自有 artifact store 后再按权限透传给前端。
