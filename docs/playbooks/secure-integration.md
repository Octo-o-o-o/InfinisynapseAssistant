...
客户端提交的 `sourceIds`、`databaseIds`、RAG 资源 ID 和文件选择都属于不可信输入。后端必须用当前已认证的 `workspaceId`/tenant 与 ID 集合重新查询并校验归属，拒绝跨工作区资源；不能让裸资源 ID、客户端传入的 `taskId` 或“当前用户能看到列表”替代服务端授权。校验后的资源集合应在发起任务前绑定成不可变选择快照，并用于后续上传、查询、prompt、产物 provenance 和恢复路径。

这条边界也适用于并行 child task：每个 child 只能使用自己的 workspace 和已授权快照，不能因为 parent 或另一个 child 已经拿到资源就扩大访问范围。
