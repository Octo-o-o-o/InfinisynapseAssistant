...
- JSON request、SSE 建连、上传、下载和业务总运行时分别设 timeout；SSE 只限制响应头建连，流本身交给 AbortSignal + idle/runtime guard。`/api/ai/message` 写入超时后先按 taskId 对账，缺少 provider 幂等保证时不要自动重放。
