...
> 事实来源：`upstream-docs/infinisynapse-site/zh/markdown/`。最后核对：2026-07-09。
...
| Skill 管理 | 安装/上传/启停用户级 Skill（active 后 Agent 可 `use_skill` 复用）；Skill 市场发现；单次任务的 `SKILL.md` 方法论上下文走任务文件链路 | `/api/ai_skill/{install,update,uninstall,toggleStatus,installedVersions,list,upload,editLocal,deleteLocal/:id}`；市场 `/skill/*`（account API base） | server-api §6 |
| 文件上传 | 三类：通用目录、任务 sandbox（响应 Agent）、任务工作区主动归档 | `/api/upload/:directory`、`/api/ai/upload?taskId=`、`/api/tools/taskUpload/:taskId` | server-api §7 |
| 工作区产物与下载 | 枚举任务产物、预览文本、二进制下载、内联渲染图片/PDF、打包 ZIP | `getTaskWorkspace/:id`、`previewFile`、`/api/tools/storage/downloadTaskFile/:taskId?path=`(+`inline=1`)、`downloadZip` | server-api §3.5、§8 |
...
| Partner SSO / 代用户调用 | 「使用 InfiniSynapse 登录」第三方接入；`withApiKey` 签发归属用户本人的 Partner API Key，代用户发起任务、计费记用户账上 | `/api/auth/partner/{sessions,token,clients*}`（account API base，`X-Client-Id`+`X-Client-Secret`） | partner-sso-integration-guide |
| CLI（agent_infini） | 终端 / Agent 工作流：任务、数据库、RAG、工作区下载；官网一键安装（CLI + companion skill）、`--update` 自动更新 | `init`、`task new/ask/cancel/ls/show/rm/file/preview/download`、`db`、`rag`、`task context` | cli-api §4；官网 CLI Install |
| 私有化部署 | Docker Compose 一键部署；初始化管理员、备份、升级、排错 | `private-deployment-guide.md` 全流程 | private-deployment |

官方已落地 mini-app 参考（高考报考、专业选校咨询、购物比价、报告快写）：`https://infinisynapse.cn/apps`；对应 API 组合模式见 server-api §11。
