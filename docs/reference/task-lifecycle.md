...
> 如果同一 SSE 连接收到其他任务的事件，必须在 SSE 消费、事件持久化、结构化输出拦截和证据/产物归属等每一层按 `data.taskId` 过滤到当前业务任务。并发子任务还应使用 `taskId + attempt`（或等价的复合键）做幂等和 provenance；不能只按业务名称或消息序号归属。缺少 `taskId` 的事件只能作为旧部署兼容路径放行，存在并发任务时不得用无作用域的 fallback。
...
>
> PLAN 终态兼容：部分环境会先把完整计划放进非空、非 partial 的 `message.say==='completion_result'`，再发送空 `message.ask==='completion_result'` 作为尾随标记。父任务仍在 PLAN 阶段时，完成标记只代表上游结束，不代表已有可审批计划；应回读 `getUiMessageById`，从已完成消息中提取并校验计划正文。只有正文非空且确实包含计划结构时才进入人工审批；只读到空标记时应 fail-closed，不能让用户审批不可见计划。
...
但“继续同一个任务”的前提是上下文没有变化：原有来源集合、prompt contract、启用的数据库/RAG、凭据/计费身份和任务权限都必须保持一致。任一项变化（例如新增或替换文件、切换数据源、改变核查口径或计费 Key）都要创建新的 `taskId`，把完整的新上下文重新绑定并保存来源快照；旧任务保持不可变，不能用旧任务的 `askResponse` 偷渡新上下文。
...
如果调用方因总超时、外部 abort、SSE 重连耗尽等原因离开消费循环，而平台任务已经发出，应 best-effort 发送一次 `cancelTask`，避免上游继续运行并消耗额度。若是优雅停机、worker 恢复或任务交接，则必须显式保留平台任务（SDK `runTask` 使用 `cancelOnExit:false`），由后续 worker 继续接管同一个 `taskId`；普通 shutdown 不能误当成用户取消。
...
- **终态不等于载荷**：`completion_result` 或 `notification.success` 可能只是空的终态标记，真实计划/结果在另一条已完成消息或 workspace 文件中。若终态正文为空，先用 `getUiMessageById` 和 `getTaskWorkspace` 恢复，再按业务 schema 验证非空载荷；不要把标记文本直接持久化为 findings、计划或正式报告。
- **增量事件读取**：服务端和 UI 应持久化最后一个 `sequence`，初次读取使用有界快照，后续用 `after_sequence=<last>` 增量追加并按事件 ID/sequence 去重；只有状态变化或显式 detail 请求才重新读取摘要。断线恢复也从最后游标开始，禁止每条 SSE 事件都从 `after_sequence=0` 重放全量历史，并为单次响应设置合理上限。
