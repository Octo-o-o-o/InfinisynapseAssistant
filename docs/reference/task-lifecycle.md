...
| `message.partial` | `{ taskId, message }` | 流式增量（同 `ts` 覆盖或追加） |
| `message.remove` | `{ taskId, messageTs: number[] }` | 移除指定消息 |
| `state.ready` | `{ taskId }` | 状态就绪，可随后 `GET /api/ai_task/tasks` 全量拉取 |
| `notification` | `{ type, title, message, duration? }` | `type==='error'` 视为任务失败 |
...
> 完成判定：收到 `message.say==='completion_result'` **或** `message.ask==='completion_result'`。两者都要判，不要只判一个。
...
4. 循环消费 SSE：
     message.partial / message.add  → 推进 UI 进度、累积文本
     notification.type==="error"    → 标记失败、结束
     message.ask==="upload_file_to_sandbox" → 见下方「Agent 请求上传」
     message.say/ask==="completion_result"  → 跳出循环
5. 读取产物（不要只用最后一条文本）：
...
```text
GET  /api/ai_task/cancelTask?taskId=<taskId>   # 或 POST
```

`taskId` 走 Query。取消后在自己业务库标记任务状态。
...
- **错误即终止**：`notification.type==='error'` 或信封 `code` 非 200 要终止并落库。
