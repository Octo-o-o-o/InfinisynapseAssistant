...
| `message.ask` | `plan_mode_response` | PLAN MODE 下的计划输出信号，可作为"等待人工审批"的完成点 |
| `message.ask` | `upload_file_to_sandbox` | Agent 请求把本地文件上传到当前任务沙箱 |
| `message.ask` | `api_req_failed` | 上游模型/余额/额度请求失败；应标记业务任务失败并落库错误 |

> 完成判定：收到 `message.say==='completion_result'` **或** `message.ask==='completion_result'`。两者都要判，不要只判一个。部分运行会以 `notification.type==='success'` + "完成" 类消息结束，也要作为完成信号兜底。两段式 plan/act 中，`message.ask==='plan_mode_response'` 表示计划可交给用户审批，不等同于 act 已完成。
