...
| 直接调 HTTP API、写 SDK、SSE、任务、数据源、RAG、Skill 管理、Partner SSO、文件上传/下载 | `infinisynapse-server-api` | `zh/markdown/server-api-reference.md`、`zh/markdown/partner-sso-integration-guide.md` |
| 使用或复刻 `agent_infini` CLI、排查 CLI 网络请求 | `infinisynapse-cli` | `zh/markdown/cli-api-reference.md` |
| 设计高考助手、购物比价、报告写作等任务型产品，判断轻量 LLM 与长任务边界 | `infinisynapse-product-patterns` | `zh/markdown/server-api-reference.md` 第 11 节 |
| Browser Use / Chrome 插件安装、购物或网页自动化产品 | `infinisynapse-browser-extension` | `zh/markdown/chrome-plugin-install.md` |

鸿蒙（HarmonyOS/ArkTS）app 集成读 `docs/playbooks/harmonyos-app-integration.md`；测试集成代码与评估 app 输出质量读 `docs/playbooks/testing-and-evaluation.md`。
...
如果用户问“最新”“现在”“今天”，先运行或建议运行 `bash tools/sync-upstream-docs.sh`，再基于本地快照作答。维护流程见 `docs/MAINTENANCE.md`。
...
- 单用户桌面 / 原生 BYOK 例外：Electron main process、Tauri command 或 native layer 可以作为本机可信后端持有用户自己的 Key，但必须存系统密钥链，renderer/WebView 只走受控 IPC / bridge，不能直接拿 Key 或直连 InfiniSynapse。详见 `docs/playbooks/desktop-native-byok.md`。
...
写 AI 产品集成时，先判断调用是否真的需要 InfiniSynapse 长任务：
...
- 新项目即使还没有自有大模型调用层，也建议先做最小 server-side LLM gateway；不要把所有轻量调用都包装成 InfiniSynapse 任务。

详细路由规则见 `docs/playbooks/llm-routing.md`。
...
- 两类 Skill 别混：长期复用的方法论装用户级 Skill（`/api/ai_skill/install|upload`，active 后 Agent 可 `use_skill`）；单次任务的 `SKILL.md` 上下文走任务文件上传链路，不进用户 Skill 库。
...
| `INF-SEC-002` | HIGH | 前端/客户端文件直连 InfiniSynapse（API Key 会进 bundle/安装包；含鸿蒙 `.ets`） |
| `INF-SSE-001` | MEDIUM | 发 `newTask` 但本文件未先连 `/api/ai/events` |
| `INF-DL-001` | MEDIUM | 把下载端点当 JSON 解析 |
| `INF-API-001` | MEDIUM | 信封成功码写成 `code===0`（应为 `200`） |
...
| 需求 | 看这里 |
| --- | --- |
| 新项目 / 老项目如何使用本规则包 | `docs/USAGE-GUIDE.md` |
| 下游业务项目如何引用并反哺本规则包 | `docs/playbooks/downstream-projects.md` |
| 平台能做什么（能力→端点） | `docs/reference/capabilities.md` |
| 上游同步 / 文档维护 / 发布前检查 | `docs/MAINTENANCE.md` |
| 端点/方法/字段定位 | `docs/reference/api-index.md` |
| 易混术语速查 | `docs/reference/glossary.md` |
| SSE 事件 / 消息类型 / 完整时序 / 恢复 | `docs/reference/task-lifecycle.md` |
| LLM 调用路由 / 轻量直连还是走 InfiniSynapse | `docs/playbooks/llm-routing.md` |
...
| 产物归档 / 自有对象存储 / 完整输出包 | `docs/playbooks/artifact-archiving.md` |
...
| 共享数据源 / 知识库市场订阅 | `docs/playbooks/market-subscriptions.md` |
| Browser Use（购物比价 / 网页自动化） | `docs/playbooks/browser-use.md` |
| 任务分享 / 公开只读结果页 | `docs/playbooks/task-sharing.md` |
| 计划/执行模式 + 高风险动作人工审批 | `docs/playbooks/plan-act-approval.md` |
...
| 无 Key 离线集成测试 | `samples/mock-server/` |
| 拿到 Key 后真实冒烟 | `samples/sdk/typescript/examples/live-smoke.ts`（会计费） |
| 一键把规则包装进下游项目 | `bash tools/install-into.sh <target>` |
...
npm test               # 扫描器 fixtures + SDK 离线单测 + mock 集成测试 + skill 镜像一致
```
